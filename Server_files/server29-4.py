...
def clientthread(conn,addr):
	try:
		type_msg = conn.recv(RECV_BUFFER)
		type_msg = type_msg.decode()
	except:
		conn.close()
		sys.exit()
	...
	if type_msg =='0':
		...
		while True:
			...
			if first_time == True:
				...
				if name_enter:
					name_pwd = conn.recv(RECV_BUFFER)
					name_pwd = name_pwd.decode() #this is the name of the newly connected client
					...
					if authenticate(name,pwd): 
						...
						conn.send('authentication_error')
						conn.close()
						sys.exit()
				else:
...
if __name__ == "__main__":
	...
	host = "10.196.23.50"  #'localhost' or '127.0.0.1' or '' are all same

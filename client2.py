...
class Application(Tkinter.Tk):
    ...
    def launch_client(self):
        ...
        self.send_button = ttk.Button(self.button_frame, text = 'Send')      #For actually sending the messages
        self.multimedia_send_button = ttk.Button(self.button_frame, text = 'Multimedia Send',command = self.multimedia_send) #For Sending Multimedia messages
        ...
        self.send_button.pack(side = Tkinter.TOP)
        self.multimedia_send_button.pack(side = Tkinter.BOTTOM)
    ...
    def initial_setup(self):
        ...
        while 1:
            if not got_list:
                try:
                    data = self.conn.recv(80000000) #Obtaining the list of all the active users
                    data = data.decode()
                    data = data.rstrip()
                except:
                    self.conn.close()
                ...
                if data == "What is your name?": #Getting the first message from the server
                    try:
                        ...
                        self.client_menu()    
                    try:
                        list_of_active_user = self.conn.recv(80000000).decode()
